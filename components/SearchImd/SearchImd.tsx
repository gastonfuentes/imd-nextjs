...
import {
  Option,
  SearchTypePropertySelect,
} from "../SearchTypeProperty/SearchTypePropertySelect";
...
export async function SearchImd() {
  ...
  const tipos: Option[] = (await fetchTipoInmuebles()).map(
    (tipo: SimpleTipoInmueble) => ({
  ...
  const ubicaciones: Option[] = (await fetchUbicaciones()).map(
    (ubi: SimpleUbicacion) => ({
