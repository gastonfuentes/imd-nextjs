...
export const fetchUbicaciones = async (): Promise<SimpleUbicacion[]> => {
    const res: UbicacionesResponse[] = await fetch("https://bisque-giraffe-421578.hostingersite.com/wp-json/wp/v2/ubicacion")
        .then((res) => res.json());
...
export const fetchUbicacionesMap = async (): Promise<Record<string, string>> => {
    const res = await fetch("https://bisque-giraffe-421578.hostingersite.com/wp-json/wp/v2/ubicacion")
        .then((res) => res.json());
